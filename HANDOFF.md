...
**TODO מיידי לסשן הבא (28.7 + תוספות מביקורת Gemini, ראו סעיף 0.2):**
- **חדש:** להוסיף תנאי RLS ב-INSERT של `Sketch` ו-`DirectMessage` שבודק `deactivated_at IS NULL` בפרופיל של המבצעת - כרגע משתמשת מושהית עדיין יכולה לשלוח DM/להעלות סקיצה, וזה פוגע בצד שלישי (מי שמקבל את ה-DM), לא רק בעצמה.
- **חדש:** לשנות את לוגיקת ה-CORS בשתי ה-Edge Functions מ-Secret קבוע (`ALLOWED_ORIGIN`) לבדיקת ה-Origin הנכנס מול תבנית regex (למשל מסתיים ב-`.app.github.dev`) - מבטל את הצורך לעדכן Secret בכל שינוי פורט.
